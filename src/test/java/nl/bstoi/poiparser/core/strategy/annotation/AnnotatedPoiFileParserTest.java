...
public class AnnotatedPoiFileParserTest {
    ...
    @Test
    public void testFieldEmbeddedRead() throws Exception {
        final String fileName = "test-excel-001.xls";
        AnnotatedPoiFileParser<EmbeddedTestRow> embeddedTestRowAnnotatedExcelFileParser = new AnnotatedPoiFileParser<EmbeddedTestRow>();
        embeddedTestRowAnnotatedExcelFileParser.setIgnoreFirstRow(false);
        final File excelFile = new File(AnnotatedPoiFileParserTest.class.getResource(filePath + fileName).toURI());
        List<EmbeddedTestRow> embeddedTestRows = embeddedTestRowAnnotatedExcelFileParser.readExcelFile(excelFile, "EmbeddedTest", EmbeddedTestRow.class);
        Assert.assertEquals("Expect 1 element in embedded test rows", 1, embeddedTestRows.size());
    ...
    @Test
    public void testGetEmbeddedPropertyDescriptions(){
        PropertyDescriptor[] x = PropertyUtils.getPropertyDescriptors(EmbeddableTestRow.class);
